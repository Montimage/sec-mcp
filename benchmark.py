...
def import_storage_v1():
    ...
    spec = importlib.util.spec_from_file_location(
        "storage", Path(__file__).parent / "sec_mcp" / "storage.py"
    )
...
def import_storage_v2():
    ...
    spec = importlib.util.spec_from_file_location(
        "storage_v2", Path(__file__).parent / "sec_mcp" / "storage_v2.py"
    )
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
...
class BenchmarkResults:
    ...
    def add_result(self, version: str, operation: str, time_ms: float, **kwargs):
        ...
        self.results[version][operation] = {"time_ms": time_ms, **kwargs}
    ...
    def print_comparison(self):
        ...
        for op in sorted(all_ops):
            ...
            speedup = f"{v1_time / v2opt_time:.1f}x" if v1_time > 0 and v2opt_time > 0 else "N/A"
            ...
            if "hot_hit_rate" in v4_results.get("domain_lookup", {}):
                ...
                for op in ["domain_lookup", "url_lookup", "ip_lookup"]:
                    if op in v4_results and "hot_hit_rate" in v4_results[op]:
                        ...
                        print(
                            f"{op.replace('_', ' ').title():<25} Hot source hit rate: {rate:.1f}%"
                        )
...
class StorageBenchmark:
    ...
    def load_data(self, data: dict[str, list[dict]]):
        ...
        print(
            f"  Loading {sum(len(v) for v in data.values())} entries into {self.version}...",
            end=" ",
            flush=True,
        )
        ...
        if data["domains"]:
            ...
            if isinstance(self.storage, StorageV2):
                ...
                if "hot_source_hits" in storage_metrics and "cold_source_hits" in storage_metrics:
                    total_hits = (
                        storage_metrics["hot_source_hits"] + storage_metrics["cold_source_hits"]
                    )
                    if total_hits > 0:
                        metrics["hot_hit_rate"] = (
                            storage_metrics["hot_source_hits"] / total_hits
                        ) * 100
...
Examples:
  ...
  python benchmark.py --v2 --v2opt         # Compare only v0.3.0 and v0.4.0
    ...
    # Data size options
    parser.add_argument(
        "--quick", action="store_true", help="Quick benchmark (10K entries, faster)"
    )
    parser.add_argument(
        "--full", action="store_true", help="Full benchmark (100K entries, production-like)"
    )

    # Version selection
    parser.add_argument(
        "--all", action="store_true", help="Benchmark all versions (v1, v0.3.0, v0.4.0)"
    )
    parser.add_argument("--v1", action="store_true", help="Benchmark v1 only")
    parser.add_argument("--v2", action="store_true", help="Benchmark v0.3.0 only")
    parser.add_argument("--v2opt", action="store_true", help="Benchmark v0.4.0 only")

    # Additional options
    parser.add_argument(
        "--memory", action="store_true", help="Include memory profiling (requires psutil)"
    )
