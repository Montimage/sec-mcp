...
@cli.command(
    help="Check a single domain, URL, or IP against the blacklist.\n\nExample: mcp check https://example.com --json"
)
...
@cli.command(
    help="Check multiple inputs from a file against the blacklist.\n\nExample: mcp batch urls.txt --json"
)
...
def batch(file: str, json: bool):
    ...
    if json:
        ...
        for value, result in zip(values, results, strict=False):
...
@cli.command(
    help="Show blacklist status (entry count, last update, sources).\n\nExample: mcp status --json"
)
@click.option("--json", is_flag=True, help="Output in JSON format")
def status(json):
